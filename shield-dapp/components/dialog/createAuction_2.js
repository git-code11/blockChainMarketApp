...
const ScheduledField = ()=>{
    ...
    const value = methods.watch('scheduled');
    return <Switch value={value} onChange={(e)=>methods.setValue('scheduled', e.target.checked)}/>
}


const FormSection = ()=>{
    const methods = useFormContext();
    const isScheduled = methods.watch('scheduled');
